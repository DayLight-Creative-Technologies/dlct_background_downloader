...
* [DLCT Fork] **iOS: restored `earliestBeginDate` for tasks with native callbacks** (`beforeTaskStart`, `onTaskStart`, `onAuth`). `URLSession` only invokes `urlSession(_:task:willBeginDelayedRequest:)` for tasks whose `earliestBeginDate` is set (Apple documented behavior); upstream commit `a698fd8` removed the lines that set it, leaving the entire iOS callback/auth-refresh hook unreachable. New helper `taskNeedsDelayedRequestHook` in `TaskFunctions.swift`; the date is set (to now) only for tasks that actually carry such callbacks.
* [DLCT Fork] **Task-record persistence is now fault-tolerant.** `_consumeUpdateTaskInDatabase` and the `trackTasks` reconciliation loop guard their database writes: a failed record write (e.g. iOS Data Protection denying file access while the device is locked, or a full disk) logs a warning instead of surfacing as an unhandled (stringified) exception on the database-updates stream. Records self-heal on the next status/progress update.
* [DLCT Fork] **Corrected misleading `Auth` documentation.** The class comment claimed `defaultOnAuth` was used as an automatic refresh fallback when no `onAuth` callback was registered; in reality no platform ever invokes any refresh without a registered callback (`hasOnAuthCallback()` gates iOS, Android, and desktop). Also added a warning about refresh-token rotation: `Auth.refreshAccessToken` rotates the refresh token inside the task's `Auth` object only, which desynchronizes session stores for providers with single-use refresh tokens (e.g. Supabase GoTrue) and can trigger family revocation.
